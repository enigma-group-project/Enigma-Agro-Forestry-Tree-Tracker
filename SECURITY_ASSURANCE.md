# Security Assurance — Enigma-Agro-Forestry-Tree-Tracker
...
| **[M] Mechanical** | Reentrancy protection on state-changers; access control enforced; CEI ordering; no unchecked external-call return; events on state change. | ✅ OZ `ReentrancyGuard` on `registerPlanting` + AuditTrail mutators; OZ `Ownable` (Steward) + Nursery role; CEI; events on every mutation. |
...
- **Design & implement:** chose a registry/attestation model (not a token); built from reviewed OZ components (`Ownable`, `ReentrancyGuard`, `Pausable`); defined roles (Steward, Nursery, Owner), state (`Status`), and transfer/revoke/renew rules.
- **Harden & verify:** enforce access control (themed `onlySteward`/`onlyTestCenter`/`onlyController`); preserve state invariants; protect external interactions (CEI + guard); test edge cases + fuzz + invariants.
...
|---|---|---|
| Reentrancy (SC05) | `registerPlanting`, AuditTrail mutators | OZ `ReentrancyGuard` + CEI (effects before any external call) |
| Broken access control (SC01) | issue / setController / revoke / pause | `onlyTestCenter` / `onlySteward` / `onlyController`; OZ `Ownable` for Steward |
| Unauthorized state change | `controller*` mutators | `onlyController`; controller is set once (`ControllerSet`) by the Steward |
| Fake issuer | `registerPlanting` | gated by `IssuerRegistry.isNursery` (Steward-granted) |
| Replay / duplicate record | `registerPlanting` | `AlreadyExists` check on the record id |
| Denial via stuck state | issuance | `Pausable` lets the Steward halt issuance during an incident, then `unpause` |
| Front-running | `revokePlanting` | issuer/Steward-only; gas price does not change correctness |
