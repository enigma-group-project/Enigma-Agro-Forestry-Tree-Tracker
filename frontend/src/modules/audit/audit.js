...
document.getElementById("transfer").onclick = async () => {
  // TODO(member4): await wc.audit.transferParcel(recordId(val("key")), val("newOwner"))
  out("TODO(member4): implement transferParcel call");
};
document.getElementById("revoke").onclick = async () => {
  // TODO(member4): await wc.audit.revokePlanting(recordId(val("key")), val("reason"))
  out("TODO(member4): implement revoke call");
};
document.getElementById("checkpoint").onclick = async () => {
  // TODO(member4): await wc.audit.logCarbonCheckpoint(recordId(val("key")), val("note"))
  out("TODO(member4): implement logCarbonCheckpoint call");
};
