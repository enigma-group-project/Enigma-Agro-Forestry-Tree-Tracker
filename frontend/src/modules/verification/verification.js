...
document.getElementById("verify").onclick = async () => {
  // TODO(member3): const [valid,status,owner,issuer] = await readContracts().verification.verifyPlanting(recordId(val("key")), hashBytes(val("artifact")));
  //               out((valid?"VALID":"NOT VALID") + ... STATUS[Number(status)] ...);
  out("TODO(member3): implement verifyPlanting call");
};
